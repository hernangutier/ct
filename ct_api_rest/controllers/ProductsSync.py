...
class ProductsSync(http.Controller):
    ...
    def createRecord(self, rec, tarifas):
        self.data.append({
            ...
            'uom_id': rec.uom_id.id if rec.uom_id else 0,
            'category_id': rec.categ_id.id if rec.categ_id else 0,
            'emin': rec.emin,
            'qty_packing': rec.qty_packing,
    ...
    # ---------- Query para Sincronizar Datos -----------------
    @http.route('/ct.api/products/UomSync/', auth='user', methods=['GET'])
    def UomSync(self):
            uom=[]
            try:
                # ----------- Consultamos los Productos -------
                records = http.request.env['uom.uom'].sudo().search([])
                for u in records:
                   uom.append({
                       'id': u.id,
                       'name': u.name
                   })
                return self.build_response(uom)
            except Exception as e:
                return self.build_response({'err', str(e)})
    ...
    def syncDepartaments(self,**kwargs):
        ...
        # ---------- Query para Sincronizar Departamentos -----------------
    @http.route('/ct.api/category/sync/', auth='user', methods=['GET'])
    def syncCategory(self, **kwargs):
            try:
                records = http.request.env['product.category'].sudo().search([])
                self.data = []
                for l in records:
                    self.data.append({
                        'id': l.id,
    ...
    def getListPrices(self, p,tarifas):
        ...
        for t in tarifas:
            price_list.append({
                ...
                'uom_id': p.uom_id.id,
                'departament_id': p.departament_id.id if p.departament_id.id else 0,
                'sku': p.default_code
            })
        return price_list

...
class ResPartner(models.Model):
    ...
    # ------- Validaciones Pasivas ----------
    @api.constrains('user_id', 'team_id')
    def _check_values(self):
        for rec in self:
            if rec.customer_rank == 1:
                if not rec.user_id: raise UserError(_("Debe asignar un Asesor de ventas..."))
                if not rec.team_id: raise UserError(_("Debe asignar una Zona de Ventas..."))
